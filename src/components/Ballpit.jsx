...
function createBallpit(e, t = {}) {
  ...
  // Safety check: ensure renderer is ready before creating spheres
  if (!i.renderer || !i.renderer.capabilities) {
    console.warn('Ballpit: Renderer capabilities not ready yet');
  } else {
    try {
      // Delay slightly to ensure context is fully active
      setTimeout(() => {
        if (!i.isDisposed) initialize(t);
      }, 10);
    } catch (err) {
      console.error('Ballpit: Initialization failed', err);
    }
  }
